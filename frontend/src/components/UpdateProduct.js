...
const UpdateProduct=()=>{
    ...
    useEffect(()=>{              {/*to get the details in console that we want to update*/}
        getProductDetails();
    },[]);
    ...
    const getProductDetails= async ()=>{
        ...
        let result = await fetch(`http://localhost:5000/product/${params.id}`,{
            headers:{
                authorization:`bearer ${JSON.parse(localStorage.getItem('token'))}`//adding jwt to this line to get product details
              }
        });
    ...
    // this is what will happen when we click on update button
    const updateProduct=async ()=>{
        ...
        let result = await fetch(`http://localhost:5000/product/${params.id}`,{
            ...
            headers:{
                ...
                authorization:`bearer ${JSON.parse(localStorage.getItem('token'))}` //adding jwt to thi sline so that update product can be opened
